...
// Only Mode/Host/Port/Username/Password are poolpayminer's to manage (they come straight from -o/-u/-p and can
// change from one run to the next); any other line already in the file -- Threads, PrimeTableLimit, whatever the
// user tuned by hand -- is read back and kept, instead of being silently wiped on the next start. Comment lines
// (including poolpayminer's own header) are dropped and regenerated fresh, everything else survives verbatim.
std::vector<std::string> readExtraRieMinerLines(const std::string &path)
{
    static const char *const kManagedKeys[] = { "mode", "host", "port", "username", "password" };
...
{
    const std::vector<std::string> extraLines = readExtraRieMinerLines(path);
    ...
    f << "# generated by poolpayminer: Mode/Host/Port/Username/Password are overwritten on every start; any other line you add here (Threads, PrimeTableLimit, ...) is kept\n"
      << "Mode = Pool\n"
      << "Host = " << host << "\n"
    ...
    for (const std::string &l : extraLines) {
        f << l << "\n";
    }
